...
/**
 * h2 database に対する `Project` (エクスポート後) に関する DAO クラス。
 * 
 * ※基本的に Project の情報は1件のみ格納します。
 */
public class H2ProjectDao {
    /**
     * このDaoが対象とするテーブルを作成。
     * 
     * @param conn データベース接続。
    ...
    public static void store2Local(Connection conn, Project source, RsvrBacklogMigToolProcessInfo processInfo)
        ...
        try (RsvrPreparedStatement stmt = RsvrJdbc
                .wrap(conn.prepareStatement("SELECT ProjectId FROM BacklogProject WHERE ProjectId=?"))) {
        ...
        if (isNew) {
            try (RsvrPreparedStatement stmtMod = RsvrJdbc
                    .wrap(conn.prepareStatement("INSERT INTO BacklogProject (ProjectId) VALUES (?)"))) {
                stmtMod.setLong(source.getId());
                stmtMod.executeUpdateSingleRow();
            }
            processInfo.incrementIns("Project");
        } else {
            processInfo.incrementUpd("Project");
        }

        // 他の項目は全てUPDATEで処理する。
        try (RsvrPreparedStatement stmtMod = RsvrJdbc.wrap(conn.prepareStatement("UPDATE BacklogProject SET " //
                + "ProjectKey=?, Name=?" //
    ...
    public static boolean isH2OnlyMyProject(Connection conn, String projectKey) throws SQLException {
        try (RsvrPreparedStatement stmt = RsvrJdbc
                .wrap(conn.prepareStatement("SELECT COUNT(ProjectId) FROM BacklogProject"))) {
            try (RsvrResultSet rset = stmt.executeQuery()) {
                rset.next();
        ...
        try (RsvrPreparedStatement stmt = RsvrJdbc
                .wrap(conn.prepareStatement("SELECT ProjectId FROM BacklogProject WHERE ProjectKey=?"))) {
    ...
    /**
     * Project Id から Project Key を取り出すための簡易メソッド。
     * 
     ...
     * @throws SQLException SQL例外が発生した場合。
     */
    public static String getProjectKey(Connection conn, long projectId) throws SQLException {
        try (RsvrPreparedStatement stmt = RsvrJdbc
                .wrap(conn.prepareStatement("SELECT ProjectKey FROM BacklogProject WHERE ProjectId =  " + projectId))) {
            try (RsvrResultSet rset = stmt.executeQuery()) {
                if (rset.next() == false) {
                    return null;
                } else {
    ...
    public static Long getProjectId(Connection conn, String projectKey) throws SQLException {
        try (RsvrPreparedStatement stmt = RsvrJdbc
                .wrap(conn.prepareStatement("SELECT ProjectKey FROM BacklogProject WHERE ProjectKey=?"))) {
            stmt.setString(projectKey);
            try (RsvrResultSet rset = stmt.executeQuery()) {
                if (rset.next() == false) {
                    return null;
