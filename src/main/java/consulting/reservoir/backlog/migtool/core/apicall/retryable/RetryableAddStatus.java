...
/**
 * Backlog API の `addStatus` をリトライ付きで呼び出すことを可能にするクラス。
 */
public class RetryableAddStatus extends AbstractRetryableApiCallout {
    private AddStatusParams params;
    private Status result;

    /**
     * コンストラクタ.
     */
    public RetryableAddStatus(AddStatusParams params) {
        this.params = params;
    }
    ...
    void processApiCallout(RsvrBacklogApiConn bklConn) throws BacklogException {
        result = bklConn.getClient().addStatus(params);
    }
